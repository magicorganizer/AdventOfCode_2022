...
class Track:
    def __init__(self, track_name, height=chr(ord("a")), pos=[0, 0], track=[]):
        ...
        for row_idx, row_list in enumerate(self.map):
            for col_idx, item in enumerate(row_list):
                if item == "a":
                    self.pos[0] = row_idx
                    self.pos[1] = col_idx
                    self.list_of_starting_points.append([row_idx, col_idx])
...
for startpoint in puzzle.list_of_starting_points:
    print(f"startpoint {startpoint}")
    puzzle.all_used_positions.clear()
    puzzle.all_explored_positions.clear()
    puzzle.tracks.clear()
    puzzle.tracks.append(Track("init_track", pos=[startpoint[0], startpoint[1]], track=[[startpoint[1], startpoint[0]]]))
    while 1:
    ...
    puzzle.get_shortest_finished_track()
    print(f"Shortest Track: {puzzle.shortest_finished_track}")
    #puzzle.print_finished_track()
